...
void QEventInfoPage::connectButtons() {
  ...
  connect(mUi->mAddButton, &QPushButton::clicked, [this]() {
    ...
    mCurrentEvent =
        new QEventItem(0, "Новое событие", crtDateTime,
                       crtDateTime.addSecs(3600)); // +1 час по умолчанию
...
void QEventInfoPage::onEventClicked(QEventItem *event) {
  ...
  if (isWorkItem && event->getId() != 0) {
    try {
      ...
      qCWarning(logEventInfo)
          << "Не удалось загрузить клиента для события:" << e.what();
    }
  }
...
void QEventInfoPage::saveEvent() {
  ...
  event.duration = (event.end_date - event.start_date) / 1000;
  ...
  if (mCreatedNewEvent || !mCurrentEvent || mCurrentEvent->getId() == 0) {
    ...
    mTimelineWidget->updateEvent(event);
    eventId = event.id;
  }
...
bool QEventInfoPage::validateInput() {
  ...
  if (isTitleEmpty) {
    QMessageBox::warning(this, tr("Ошибка"),
                         tr("Название события не может быть пустым"));
    return false;
  }
  if (isZeroDuration) {
    QMessageBox::warning(
