...
QEventView::QEventView(QWidget *parent) : QGraphicsView(parent), mModel(nullptr) {
...
void QEventView::setModel(QTimelineModel *model) {
    ...
    connect(mModel, &QTimelineModel::rowsInserted,
            this, &QEventView::onRowsInserted);
    connect(mModel, &QTimelineModel::rowsRemoved,
            this, &QEventView::onRowsRemoved);
    connect(mModel, &QTimelineModel::dataChanged,
            this, &QEventView::onDataChanged);
    connect(mModel, &QTimelineModel::modelReset,
            this, &QEventView::onModelReset);
}


void QEventView::onRowsInserted(const QModelIndex &parent, int first, int last) {
    Q_UNUSED(parent)
    ...
    for (int row = first; row <= last; ++row) {
        ...
        if (!var.canConvert<ObxEvent>()) continue;

        ObxEvent event = var.value<ObxEvent>();
        QEventItem *item = new QEventItem(event);

        connect(item, &QEventItem::itemSelected, this, &QEventView::onEventSelected);

        mScene->addItem(item);
        mSceneItems.insert(event.id, item); // Сохраняем для быстрого доступа
    }
...
void QEventView::onRowsRemoved(const QModelIndex &parent, int first, int last) {
    ...
    for (int row = first; row <= last; ++row) {
        ...
        obx_id eventId = var.value<qint64>(); // IdRole возвращает qint64
...
void QEventView::onDataChanged(
    const QModelIndex &topLeft,
    const QModelIndex &bottomRight,
    const QList<int> &roles) {
    Q_UNUSED(roles) // Пока обновляем всё, можно оптимизировать по ролям
    ...
    for (int row = topLeft.row(); row <= bottomRight.row(); ++row) {
        ...
        auto it = mSceneItems.find(eventId);
        if (it != mSceneItems.end()) {
            ObxEvent updatedEvent = index.data(QTimelineModel::EventDataRole).value<ObxEvent>();
...
void QEventView::onModelReset() {
    ...
    QModelIndex index = mModel->index(row, 0, QModelIndex());
        ...
        ObxEvent event = index.data(QTimelineModel::EventDataRole).value<ObxEvent>();
        QEventItem *item = new QEventItem(event);
        connect(item, &QEventItem::itemSelected, this, &QEventView::onEventSelected);
        mScene->addItem(item);
