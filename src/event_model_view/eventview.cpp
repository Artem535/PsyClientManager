...
void EventView::updateSceneSize() {
  ...
  mPixelPerMin = static_cast<qreal>(sceneHeight) / pcm::widgets::constants::kMinInDay;
...
void EventView::addDemoItems() {
  ...
  mEvents.push_back(new EventItem{"Meeting", now, now.addSecs(3600), QSize(itemWidth, 100), true});
  mEvents.push_back(new EventItem{"Lunch", now.addSecs(7200), now.addSecs(9000), QSize(itemWidth, 80), false});
